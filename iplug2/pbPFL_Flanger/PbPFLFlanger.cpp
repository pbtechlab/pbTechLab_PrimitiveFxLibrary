...
PbPFLFlanger::PbPFLFlanger(const InstanceInfo& info)
: Plugin(info, MakeConfig(kNumParams, kNumPresets))
{
  // Parameter set is a 1:1 match of the original JUCE Flanger (ranges, defaults,
  // steps and units are identical, so nothing is lost in the migration).
  GetParam(kRate)->InitDouble("Rate", 0.32, 0.03, 8.0, 0.01, "Hz");
  GetParam(kDepth)->InitDouble("Depth", 55.0, 0.0, 100.0, 1.0, "%");
  GetParam(kDelay)->InitDouble("Delay", 4.5, 0.1, 15.0, 0.1, "ms");
  GetParam(kFeedback)->InitDouble("Feedback", 42.0, -95.0, 95.0, 1.0, "%");
  GetParam(kSpread)->InitDouble("Spread", 135.0, 0.0, 180.0, 1.0, "deg");
  ...
  MakePreset("Flanger Init", 0.32, 55.0, 4.5, 42.0, 135.0, 50.0, 0.0, 0.0);
}

#if IPLUG_DSP
void PbPFLFlanger::OnReset()
{
  for (auto& d : mDelay)
...
void PbPFLFlanger::ProcessBlock(sample** inputs, sample** outputs, int nFrames)
{
  ...
  const float base     = static_cast<float>(GetParam(kDelay)->Value()) * static_cast<float>(sr) * 0.001f;
  const float depth    = static_cast<float>(GetParam(kDepth)->Value()) * 0.01f * base;
  const float fb       = static_cast<float>(GetParam(kFeedback)->Value()) * 0.01f;
  const float spread   = static_cast<float>(GetParam(kSpread)->Value()) / 180.0f;
  ...
  for (int i = 0; i < nFrames; ++i)
  {
    ...
    for (int ch = 0; ch < nChans; ++ch)
    {
      const float phase = mLfoPhase + (ch == 0 ? 0.0f : spread * 0.5f);
      float d = base + std::sin(phase * kTwoPi) * depth;
      d = std::clamp(d, 1.0f, 191999.0f);

      const float dry = static_cast<float>(inputs[ch][i]);
      const float delayed = mDelay[ch].Read(d);
      mDelay[ch].Write(dry + delayed * fb);
...
bool PbPFLFlanger::OnMessage(int msgTag, int ctrlTag, int dataSize, const void* pData)
{

...
PbPFLVibrato::PbPFLVibrato(const InstanceInfo& info)
: Plugin(info, MakeConfig(kNumParams, kNumPresets))
{
  // Parameter set is a 1:1 match of the original JUCE Vibrato (ranges, defaults,
  // steps and units are identical, so nothing is lost in the migration).
  GetParam(kRate)->InitDouble("Rate", 5.2, 0.03, 120.0, 0.01, "Hz");
  GetParam(kDepth)->InitDouble("Depth", 35.0, 0.0, 100.0, 1.0, "%");
  GetParam(kDelay)->InitDouble("Delay", 7.0, 1.0, 20.0, 0.1, "ms");
  GetParam(kShape)->InitDouble("Shape", 50.0, 0.0, 100.0, 1.0, "%");
  GetParam(kStereo)->InitDouble("Stereo", 90.0, 0.0, 180.0, 1.0, "deg");
  ...
  MakePreset("Vibrato Init", 5.2, 35.0, 7.0, 50.0, 90.0, 100.0, 0.0);
}

#if IPLUG_DSP
void PbPFLVibrato::OnReset()
{
  for (auto& d : mDelay)
...
void PbPFLVibrato::ProcessBlock(sample** inputs, sample** outputs, int nFrames)
{
  ...
  const float baseDelay    = static_cast<float>(GetParam(kDelay)->Value()) * static_cast<float>(sr) * 0.001f;
  const float depth        = static_cast<float>(GetParam(kDepth)->Value()) * static_cast<float>(sr) * 0.00008f;
  const float shape        = static_cast<float>(GetParam(kShape)->Value()) * 0.01f;
  const float stereoOffset = static_cast<float>(GetParam(kStereo)->Value()) / 360.0f;
  ...
  for (int i = 0; i < nFrames; ++i)
  {
    ...
    for (int ch = 0; ch < nChans; ++ch)
    {
      const float phase = mLfoPhase + (ch == 0 ? 0.0f : stereoOffset);
      const float sine = std::sin(phase * kTwoPi);
      const float triangle = 2.0f * std::fabs(2.0f * (phase - std::floor(phase + 0.5f))) - 1.0f;
...
bool PbPFLVibrato::OnMessage(int msgTag, int ctrlTag, int dataSize, const void* pData)
{

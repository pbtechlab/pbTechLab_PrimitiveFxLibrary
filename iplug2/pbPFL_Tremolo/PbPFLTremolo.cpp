...
PbPFLTremolo::PbPFLTremolo(const InstanceInfo& info)
: Plugin(info, MakeConfig(kNumParams, kNumPresets))
{
  // Parameter set is a 1:1 match of the original JUCE Tremolo (ranges, defaults,
  // steps and units are identical, so nothing is lost in the migration).
  GetParam(kRate)->InitDouble("Rate", 4.0, 0.03, 120.0, 0.01, "Hz");
  GetParam(kDepth)->InitDouble("Depth", 65.0, 0.0, 100.0, 1.0, "%");
  GetParam(kShape)->InitDouble("Shape", 45.0, 0.0, 100.0, 1.0, "%");
  GetParam(kStereo)->InitDouble("Stereo", 0.0, 0.0, 180.0, 1.0, "deg");
  GetParam(kSmooth)->InitDouble("Smooth", 20.0, 0.0, 100.0, 1.0, "%");
  ...
  MakePreset("Tremolo Init", 4.0, 65.0, 45.0, 0.0, 20.0, 100.0, 0.0, 0.0);
}

#if IPLUG_DSP
void PbPFLTremolo::OnReset()
{
  mLfoPhase = 0.0f;
  mSmoothL = 0.0f;
  mSmoothR = 0.0f;
...
void PbPFLTremolo::ProcessBlock(sample** inputs, sample** outputs, int nFrames)
{
  ...
  const float stereoOff   = static_cast<float>(GetParam(kStereo)->Value()) / 360.0f;
  const float smoothCoeff = 0.04f + static_cast<float>(GetParam(kSmooth)->Value()) * 0.008f;
  ...
  for (int i = 0; i < nFrames; ++i)
  {
    ...
    for (int ch = 0; ch < nChans; ++ch)
    {
      const float phase = mLfoPhase + (ch == 0 ? 0.0f : stereoOff);
      const float sine = 0.5f + 0.5f * std::sin(phase * kTwoPi);
      const float square = sine > 0.5f ? 1.0f : 0.0f;
      const float lfo = sine * (1.0f - shape) + square * shape;
      const float target = 1.0f - depth * lfo;

      float& sm = (ch == 0) ? mSmoothL : mSmoothR;
      sm = Smooth(sm, target, smoothCoeff);

      const float dry = static_cast<float>(inputs[ch][i]);
      const float wet = dry * sm;
...
bool PbPFLTremolo::OnMessage(int msgTag, int ctrlTag, int dataSize, const void* pData)
{
